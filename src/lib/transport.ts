...
export default class Transport extends EventEmitter {
	...
	private ownAddress: { address: string; port: number } = {
		address: '',
		port: 0,
	}
	...
	constructor(settings: TransportSettings) {
		...
		this._server.on('message', (msg, rinfo) => {
			...
			if (this.ownAddress.port === rinfo.port) {
				for (const [messageKey, earlierSentBuffer] of Object.entries(
					this._lastSendMessages,
				)) {
					if (msg.equals(earlierSentBuffer)) {
						debug(
							`server IGNORE message from ${rinfo.address}:${rinfo.port} (${messageKey}): ${msg.toString('hex')}`,
						)
						return
					}
				}
			...
			debug(
				`server got message from ${rinfo.address}:${rinfo.port}: ${msg.toString('hex')}`,
			)
			this.emit(
				'message',
				msg,
	...
	send(buffer: Buffer, offset: number, receiver?: string): void {
		...
		debug(
			`Send packet to ${receiver}: ${buffer
				.toString('hex')
		...
		this._server.send(
			...
			0,
			offset,
			parseInt(port) || DEFAULT_BACNET_PORT,
			address,
		)
	}
