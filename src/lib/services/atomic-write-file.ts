...
export const decode = (buffer: Buffer, offset: number, apduLen: number) => {
  ...
  const objectId: BACNetObjectID = {type: decodedValue.objectType, instance: decodedValue.instance};
...
export const encodeAcknowledge = (buffer: EncodeBuffer, isStream: boolean, position: number): void => {
  if (isStream) {
