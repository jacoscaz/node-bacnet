...
export const encode = (buffer: EncodeBuffer, objectId: BACNetObjectID, values: any[]): void => {
...
export const decode = (buffer: Buffer, offset: number, apduLen: number) => {
  ...
  if (!baAsn1.decodeIsOpeningTagNumber(buffer, offset + len, 1)) return undefined;
  len++;
...
export const encodeAcknowledge = (buffer: EncodeBuffer, objectId: BACNetObjectID): void => {
