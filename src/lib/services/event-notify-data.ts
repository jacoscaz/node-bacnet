...
export const decode = (buffer: Buffer, offset: number) => {
  ...
  if (!baAsn1.decodeIsContextTag(buffer, offset + len, 2)) return undefined;
  result = baAsn1.decodeTagNumberAndValue(buffer, offset + len);
  len += result.len;
  decodedValue = baAsn1.decodeObjectId(buffer, offset + len);
  len += decodedValue.len;
  ...
  if (!baAsn1.decodeIsContextTag(buffer, offset + len, 3)) return undefined;
  len += 2;
  ...
  if (!baAsn1.decodeIsContextTag(buffer, offset + len, 4)) return undefined;
  result = baAsn1.decodeTagNumberAndValue(buffer, offset + len);
  len += result.len;
  decodedValue = baAsn1.decodeUnsigned(buffer, offset + len, result.value);
  len += decodedValue.len;
  ...
  if (!baAsn1.decodeIsContextTag(buffer, offset + len, 5)) return undefined;
  result = baAsn1.decodeTagNumberAndValue(buffer, offset + len);
  len += result.len;
  decodedValue = baAsn1.decodeUnsigned(buffer, offset + len, result.value);
  len += decodedValue.len;
  ...
  if (!baAsn1.decodeIsContextTag(buffer, offset + len, 6)) return undefined;
  result = baAsn1.decodeTagNumberAndValue(buffer, offset + len);
  len += result.len;
  ...
  if (!baAsn1.decodeIsContextTag(buffer, offset + len, 8)) return undefined;
  result = baAsn1.decodeTagNumberAndValue(buffer, offset + len);
  len += result.len;
  decodedValue = baAsn1.decodeEnumerated(buffer, offset + len, result.value);
  len += decodedValue.len;
  ...
  switch (eventData.notifyType) {
    ...
    case baEnum.NotifyType.EVENT:
      ...
      if (!baAsn1.decodeIsContextTag(buffer, offset + len, 10)) return undefined;
      result = baAsn1.decodeTagNumberAndValue(buffer, offset + len);
      len += result.len;
      decodedValue = baAsn1.decodeEnumerated(buffer, offset + len, result.value);
      len += decodedValue.len;
  ...
  if (!baAsn1.decodeIsContextTag(buffer, offset + len, 11)) return undefined;
  result = baAsn1.decodeTagNumberAndValue(buffer, offset + len);
  len += result.len;
  decodedValue = baAsn1.decodeEnumerated(buffer, offset + len, result.value);
  len += decodedValue.len;
