"use strict";
var __createBinding = (this && this.__createBinding) || (Object.create ? (function(o, m, k, k2) {
    if (k2 === undefined) k2 = k;
    var desc = Object.getOwnPropertyDescriptor(m, k);
    if (!desc || ("get" in desc ? !m.__esModule : desc.writable || desc.configurable)) {
      desc = { enumerable: true, get: function() { return m[k]; } };
    }
    Object.defineProperty(o, k2, desc);
}) : (function(o, m, k, k2) {
    if (k2 === undefined) k2 = k;
    o[k2] = m[k];
}));
var __setModuleDefault = (this && this.__setModuleDefault) || (Object.create ? (function(o, v) {
    Object.defineProperty(o, "default", { enumerable: true, value: v });
}) : function(o, v) {
    o["default"] = v;
});
var __importStar = (this && this.__importStar) || (function () {
    var ownKeys = function(o) {
        ownKeys = Object.getOwnPropertyNames || function (o) {
            var ar = [];
            for (var k in o) if (Object.prototype.hasOwnProperty.call(o, k)) ar[ar.length] = k;
            return ar;
        };
        return ownKeys(o);
    };
    return function (mod) {
        if (mod && mod.__esModule) return mod;
        var result = {};
        if (mod != null) for (var k = ownKeys(mod), i = 0; i < k.length; i++) if (k[i] !== "default") __createBinding(result, mod, k[i]);
        __setModuleDefault(result, mod);
        return result;
    };
})();
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
const node_test_1 = __importDefault(require("node:test"));
const node_assert_1 = __importDefault(require("node:assert"));
const utils = __importStar(require("./utils"));
const src_1 = require("../../src");
node_test_1.default.describe('bacnet - readPropertyMultiple integration', () => {
    (0, node_test_1.default)('should return a timeout error if no device is available', (t) => {
        return new Promise((resolve) => {
            const client = new utils.BacnetClient({ apduTimeout: 200 });
            const requestArray = [
                {
                    objectId: { type: 8, instance: 4194303 },
                    properties: [{ id: 8, index: src_1.ASN1_ARRAY_ALL }],
                },
            ];
            client.readPropertyMultiple('127.0.0.2', requestArray, {}, (err, value) => {
                node_assert_1.default.strictEqual(err.message, 'ERR_TIMEOUT');
                node_assert_1.default.strictEqual(value, undefined);
                client.close();
                resolve();
            });
        });
    });
    (0, node_test_1.default)('should successfully decode a structured view', (t) => {
        return new Promise((resolve) => {
            const transport = new utils.TransportStub();
            const client = new utils.BacnetClient({ transport });
            const data = Buffer.from('', 'hex');
            const requestArray = [
                {
                    objectId: { type: 8, instance: 4194303 },
                    properties: [{ id: 8, index: src_1.ASN1_ARRAY_ALL }],
                },
            ];
            client.readPropertyMultiple('127.0.0.2', requestArray, {}, (err, response) => {
                node_assert_1.default.strictEqual(err, null);
                const object = utils.propertyFormater(response.values[0].values);
                node_assert_1.default.deepStrictEqual(response.values[0].objectId, {
                    type: 29,
                    instance: 31,
                });
                node_assert_1.default.deepStrictEqual(object[28], [
                    {
                        value: 'Collection of lighting presence detector',
                        type: 7,
                        encoding: 0,
                    },
                ]);
                node_assert_1.default.deepStrictEqual(object[75], [
                    { value: { type: 29, instance: 31 }, type: 12 },
                ]);
                node_assert_1.default.deepStrictEqual(object[77], [
                    {
                        value: "Zählerweg'Floor 4'RSegTV'Lgt'PscOp'LgtPscDetCol",
                        type: 7,
                        encoding: 0,
                    },
                ]);
                node_assert_1.default.deepStrictEqual(object[79], [{ value: 29, type: 9 }]);
                node_assert_1.default.deepStrictEqual(object[168], [
                    {
                        value: '7-BA-RDS1-041-SBCv13.20',
                        type: 7,
                        encoding: 0,
                    },
                ]);
                node_assert_1.default.deepStrictEqual(object[207], [
                    { value: '', type: 7, encoding: 0 },
                ]);
                node_assert_1.default.deepStrictEqual(object[208], [{ value: 8, type: 9 }]);
                node_assert_1.default.deepStrictEqual(object[210], [
                    { value: 'LgtPscDetRs', type: 7, encoding: 0 },
                    { value: 'PscDet(*)', type: 7, encoding: 0 },
                ]);
                node_assert_1.default.deepStrictEqual(object[211], [
                    {
                        value: {
                            value: { type: 5, instance: 0 },
                            type: 12,
                        },
                        type: 118,
                    },
                    {
                        value: {
                            value: { type: 3, instance: 0 },
                            type: 12,
                        },
                        type: 118,
                    },
                ]);
                node_assert_1.default.deepStrictEqual(object[4930], [
                    { value: 0, type: 9 },
                ]);
                node_assert_1.default.deepStrictEqual(object[4941], [
                    { value: 6, type: 9 },
                ]);
                node_assert_1.default.deepStrictEqual(object[5030], [
                    { value: 0, type: 2 },
                ]);
                node_assert_1.default.deepStrictEqual(object[5053], [
                    { value: 0, type: 9 },
                ]);
                node_assert_1.default.deepStrictEqual(object[5086], [
                    { value: { value: [1], bitsUsed: 1 }, type: 8 },
                    { value: { value: [0], bitsUsed: 1 }, type: 8 },
                ]);
                node_assert_1.default.deepStrictEqual(object[5102], [
                    { value: 41, type: 9 },
                ]);
                client.close();
                resolve();
            });
            transport.emit('message', data, '127.0.0.2');
        });
    });
    (0, node_test_1.default)('should successfully decode a value object', (t) => {
        return new Promise((resolve) => {
            const transport = new utils.TransportStub();
            const client = new utils.BacnetClient({ transport });
            const data = Buffer.from('', 'hex');
            const requestArray = [
                {
                    objectId: { type: 8, instance: 4194303 },
                    properties: [{ id: 8, index: src_1.ASN1_ARRAY_ALL }],
                },
            ];
            client.readPropertyMultiple('127.0.0.2', requestArray, {}, (err, response) => {
                node_assert_1.default.strictEqual(err, null);
                node_assert_1.default.deepStrictEqual(response.values[0].objectId, {
                    type: 19,
                    instance: 10,
                });
                const object = utils.propertyFormater(response.values[0].values);
                node_assert_1.default.deepStrictEqual(object[28], [
                    { value: 'Sensor', type: 7, encoding: 0 },
                ]);
                node_assert_1.default.deepStrictEqual(object[36], [{ value: 0, type: 9 }]);
                node_assert_1.default.deepStrictEqual(object[74], [{ value: 8, type: 2 }]);
                node_assert_1.default.deepStrictEqual(object[75], [
                    { value: { type: 19, instance: 10 }, type: 12 },
                ]);
                node_assert_1.default.deepStrictEqual(object[77], [
                    {
                        value: "Zählerweg'Floor 4'RSegTV'SenDev'Sen",
                        type: 7,
                        encoding: 0,
                    },
                ]);
                node_assert_1.default.deepStrictEqual(object[79], [{ value: 19, type: 9 }]);
                node_assert_1.default.deepStrictEqual(object[81], [
                    { value: false, type: 1 },
                ]);
                node_assert_1.default.deepStrictEqual(object[85], [{ value: 1, type: 2 }]);
                node_assert_1.default.deepStrictEqual(object[103], [{ value: 0, type: 9 }]);
                node_assert_1.default.deepStrictEqual(object[110], [
                    { value: 'Operational', type: 7, encoding: 0 },
                    { value: 'Device stopped', type: 7, encoding: 0 },
                    { value: 'Device not assigned', type: 7, encoding: 0 },
                    { value: 'Device missing', type: 7, encoding: 0 },
                    { value: 'Configuring device', type: 7, encoding: 0 },
                    { value: 'Unused', type: 7, encoding: 0 },
                    {
                        value: 'Missing or wrong configuration',
                        type: 7,
                        encoding: 0,
                    },
                    { value: 'Searching', type: 7, encoding: 0 },
                ]);
                node_assert_1.default.deepStrictEqual(object[111], [
                    { value: { value: [0], bitsUsed: 4 }, type: 8 },
                ]);
                node_assert_1.default.deepStrictEqual(object[168], [
                    {
                        value: '7-BA-RDS1-024-SBCv13.20',
                        type: 7,
                        encoding: 0,
                    },
                ]);
                node_assert_1.default.deepStrictEqual(object[4930], [
                    { value: 0, type: 9 },
                ]);
                node_assert_1.default.deepStrictEqual(object[4941], [
                    { value: 5, type: 9 },
                ]);
                node_assert_1.default.deepStrictEqual(object[5000], [
                    { value: 0, type: 9 },
                ]);
                node_assert_1.default.deepStrictEqual(object[5001], [
                    { value: 0, type: 9 },
                ]);
                node_assert_1.default.deepStrictEqual(object[5039], [
                    { value: '0.2.249', type: 7, encoding: 0 },
                ]);
                node_assert_1.default.deepStrictEqual(object[5087], [
                    { value: '5WG1258-2DB12', type: 7, encoding: 0 },
                ]);
                node_assert_1.default.deepStrictEqual(object[5092], [
                    { value: 'PL-1:DL=1;', type: 7, encoding: 0 },
                ]);
                node_assert_1.default.deepStrictEqual(object[5094], [
                    { value: 3, type: 2 },
                ]);
                node_assert_1.default.deepStrictEqual(object[5100], [
                    { value: '00010043eddc', type: 7, encoding: 0 },
                ]);
                node_assert_1.default.deepStrictEqual(object[5101], [
                    {
                        value: 'PL:DDT=0586.0001.00.01.00;FW=0.1.13;MODE=PL;',
                        type: 7,
                        encoding: 0,
                    },
                ]);
                node_assert_1.default.deepStrictEqual(object[5102], [
                    { value: 24, type: 9 },
                ]);
                node_assert_1.default.deepStrictEqual(object[5103], [
                    { value: '', type: 7, encoding: 0 },
                ]);
                node_assert_1.default.deepStrictEqual(object[5104], [
                    { value: 0, type: 9 },
                ]);
                node_assert_1.default.deepStrictEqual(object[5107], [
                    { value: 1, type: 2 },
                ]);
                client.close();
                resolve();
            });
            transport.emit('message', data, '127.0.0.2');
        });
    });
});
//# sourceMappingURL=read-property-multiple.spec.js.map